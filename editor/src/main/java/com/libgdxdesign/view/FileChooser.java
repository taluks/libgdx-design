 ...
 * implementation is not available. For "desktop" device, the standard file
 * chooser that comes with Swing is far more functional. Android doesn't really
 * have a standard one, but there are open source android libraries that provide
 * this that are far more functional than this libgdx implementation. But it is
...
public class FileChooser extends BaseDialog {
	...
	public FileChooser(String title, final Skin skin, FileHandle baseDir) {
		...
		getButtonTable().defaults().minWidth(240).pad(10);
	...
	public Dialog show(Stage stage, Action action) {
		final Table content = getContentTable();
		content.add(fileListLabel).width(500).padTop(4).row();
		...
		if (fileNameEnabled) {
			content.add(fileNameLabel).width(500).expandX().row();
			content.add(fileNameInput).width(500).expandX().row();
